...
struct pack_for_moving
{
    struct player_t *P;
    struct player_info_t *P_inf;
};
struct room
{
   int player[4];
...
void print_statistics(struct player_info_t * S)
{
    ...
    if(S->camp_x != 0 && S->camp_y !=0)
        mvprintw(2,60,"Campsite X/Y:%3d/%3d",S->camp_x,S->camp_y);
    else    
        mvprintw(2,60,"Campsite X/Y:unknown");
    mvprintw(3,60,"Round number:%7d",S->round);
...
void print_legend()
{
   ...
   mvprintw(17,63," - move pleayer");
   mvprintw(18,63," - players");
   mvprintw(19,63," - wall");
   mvprintw(20,63," - bushes");
...
void *moving(void * arg) //losuje pozycje i wpisuje znak "w,a,s,d"
{
    
    struct pack_for_moving * S = (struct pack_for_moving*)arg;
    ...
    {   
       ...
       switch(k)
       {
           case 0:
            ...
            if(S->P_inf->map[1][2] != '0' +1)
            {
                int flag=0;
                for(int k=0;k<10;k++)
                {
                    if(tab[k][0] == S->P_inf->x - 1 &&  tab[k][1] == S->P_inf->y )
                        flag =1;
                ...
                {
                    temp=0;
                    tab[i%10][0] = S->P_inf->x;
                    tab[i%10][1] = S->P_inf->y;   
                    i++;
                    S->P->key ='w';
                }
                if(flag == 1)
             ...
             break;
           case 1:
                 if(S->P_inf->map[2][3] != '0' +1 )   
        {
             int flag=0;
            for(int k=0;k<10;k++)
            {
                if(tab[k][0] == S->P_inf->x  && tab[k][1] == S->P_inf->y + 1 )
                {
                    flag =1;
...
struct pack_for_moving S_mov;
S_mov.P = player;
S_mov.P_inf = player_inf;
pthread_t th,th2;



pthread_create(&th2,NULL,printing_map,player_inf); 
pthread_create(&th,NULL,moving,&S_mov);
